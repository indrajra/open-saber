...
var populate_add_tasks = function (tasks, entityType, static_payload, arrDynamicData, someEntity) {
    ...
    for (var itr = 0; itr < arrDynamicData.length; itr++) {
        ...
        for (var field in dataPortion) {
            ...
            if (field === 'DOB') {
                var DOB = dataPortion[field]
                DOB = dateFormat(DOB, 'yyyy-mm-dd')
                dataPortion[field] = DOB
            }

            if (field === 'AdmissionDate') {
                var adminissionDate = dataPortion[field]
                adminissionDate = dateFormat(DOB, 'yyyy-mm-dd')
                dataPortion[field] = adminissionDate
            }
        }
